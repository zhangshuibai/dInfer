...
ouput_dir='./outputs' # your custom output directory
gpus='0;1;2;3' # gpus for tensor parallel inference
parallel='tp' # 'tp' for tensor parallel or 'dp' for data parallel
model_type='llada_moe' # llada2 (for llada2-mini) | llada_moe (for llada-moe) | llada (for llada or llada-1.5)
...
if [ parallel=='tp' ]; then
  for task in gsm8k_llada_moe mbpp_sanitized_llada1.5; do
